...
Adafruit_NeoPixel SpinStrip = Adafruit_NeoPixel(N_LEDS, SpinLEDPin, NEO_GRB + NEO_KHZ800); //number of LEDs might be dif
...
void loop(){ //havent implemented interrupt for squeeze
  ...
  SqueezeState = updateState(SqueezeReading, SqueezeState);
  SpinState = updateState(SqueezeReading, SpinState);
//  SqueezeState = updateSqueezeState(SqueezeReading, SqueezeState);
//  SpinState = updateSpinState(SqueezeReading, SpinState);
  ...
  if (SpinState != old_SpinState) { //updates flag to allow motor to run only when state has changed
    squeeze_motor_flag = 0;
  }

  if (TapReading <= SqueezeReading && TapReading <= SpinReading) { //output
    Tap(true);//function for output with only tap make noise
    Squeeze(false);
    Spin(false);
  }
  else if (SqueezeReading <= TapReading && SqueezeReading <= SpinReading){
    Tap(false);
    Squeeze(true);//function for output with only squeeze make noise
    Spin(false);
  }
  else {
    Tap(false);
    Squeeze(false);
    Spin(true); //function for output with only spin make noise
  }

  old_TapState = TapState;
  old_SqueezeState = SqueezeState;
  old_SpinState = SpinState;
  //---------------------CHANGES-----------------------
}
...
void Spin(int noise) {
  ...
  if(SpinState == 0){
    ...
    analogWrite(SpinMotorPin, 0);
    Serial.print("Spin sensor not triggered \n");
    ...
    nextPSpin = 100; //do not play any note. 100 was arbitrary choice
    digitalWrite(SpinIntrptPin, LOW);

  }
  else if(SpinState == 1){
    stop = false; //starts the timer
    for(int i=0;i <N_LEDS;i++) {
      SpinStrip.setPixelColor(i, SpinStrip.Color(255,0,255)); //purple
    }
    SpinStrip.show(); //Sends updated pixel color to hardware
    nextPSpin = 2;
    // --------------------- MOTOR LOGIC -----------------------
    if(spin_motor_flag == 1){ //check if sensor was also triggered last loop
        // sensor was pressed last loop, so check motor time. do nothing if motor time is less
        if((currentMillis - previousMillisSpin) > interval){
          // motor interval has passed, so need to turn off motor
          analogWrite(SpinMotorPin, 0);
        }
    }
      ...
      analogWrite(SpinMotorPin, 255); // PWM
      spin_motor_flag = 1; //store tap_flag status for next loop
    }
    // --------------------- MOTOR LOGIC -----------------------
  }
  else if(SpinState == 2){
    stop = false; //starts the timer
    for(int i=0;i <N_LEDS;i++) {
      SpinStrip.setPixelColor(i, SpinStrip.Color(255,255,0)); //yellow
    }
    SpinStrip.show(); //Sends updated pixel color to hardware
    nextPSpin = 1;
    // --------------------- MOTOR LOGIC -----------------------
    ...
    stop = false; //starts the timer
    for(int i=0;i <N_LEDS;i++) {
      SpinStrip.setPixelColor(i, SpinStrip.Color(0,255,255)); //blue
    }
    SpinStrip.show(); //Sends updated pixel color to hardware
    nextPSpin = 0;
    // --------------------- MOTOR LOGIC -----------------------
  ...
  // ------------------- MUSIC LOGIC ----------------------------
  if(nextPSpin != currentPSpin && nextPSpin - currentPSpin < 0 && noise == true) {
      playNote(nextPSpin);
    }
    currentPSpin = nextPSpin;

    stop = true;
  // ------------------- MUSIC LOGIC -------------------
  // ------------------- Interrupt LOGIC -------------------

  if(SpinReading < PURPLE){  //Checks if there is any pressure detected by sensor
    digitalWrite(SpinIntrptPin, HIGH);  // If it's within the thresholds, set the SpinInterruptPin to logic HIGH
  }
}
